...
def grade_predictions(
    ...
    for row in merged.itertuples(index=False):
        ...
        away_spread = _safe_float(row.away_spread)
        home_spread = _safe_float(row.home_spread)
        total_line = _safe_float(row.total_line)
...
def filter_ungraded_predictions(
    ...
    if predictions.empty or grades.empty or "prediction_id" not in grades.columns:
        return predictions
